...
class BSP_AE(object):
    ...
    def get_shape_from_z(self, z_vector, idx, config):
        w2 = self.bsp_network.generator.convex_layer_weights.detach().cpu().numpy()
        model_float = np.ones([self.real_size, self.real_size, self.real_size, self.c_dim], np.float32)
        model_float_combined = np.ones([self.real_size, self.real_size, self.real_size], np.float32)
        out_m = self.bsp_network.decoder(z_vector, is_training=False)
        dima = self.test_size
        dim = self.real_size
        multiplier = int(dim / dima)
        multiplier2 = multiplier * multiplier
        ...
        vertices, polygons = get_mesh_watertight(bsp_convex_list)
        # output ply
        write_ply_polygon(config.sample_dir + "/" + str(idx) + "_bsp.ply", vertices, polygons)

    def eval_z(self, config, z_vectors):
        if not self.load(): exit(-1)
        for idx in range(z_vectors.shape[0]):
            z_vector = z_vectors[idx:idx + 1, :]
            self.get_shape_from_z(z_vector, idx, config)
